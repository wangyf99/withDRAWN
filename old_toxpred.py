...
def get_prroc_averages(prroc_conds,classifiers, cl, outdir, rs_in, write=False):
    ...
    for clf_i in range(len(classifiers)):
        if 'all-'+cl[clf_i] in prroc_conds:
            train_data = pd.read_csv(outdir+str(rs_in)+'-level2_train.csv', header=None)
            train_data = train_data.transpose()
            Ltrain = get_label_from_l1(outdir, 'train')
            clf = classifiers[clf_i]
            clf.fit(train_data,np.array(Ltrain))
            ...
            for rs in range(rs_in, rs_in +10):
                train_data = pd.read_csv(outdir+str(rs)+'-level2_train.csv', header=None)
                train_data = train_data.transpose()
                test_data = pd.read_csv(outdir+str(rs)+'-level2_test.csv', header=None)
                test_data = test_data.transpose()
                ...
                filename = outdir + 'prroc/'+str(rs)+'all-'+cl[clf_i]+'-curves.csv'
                with open(filename,"w") as f:
                    f.write("\n".join(",".join(map(str, x)) for x in (fpr,tpr,precision, recall)))
            ...
            #         fout0 = open(outdir+'prroc/level2_summary.csv', '+a')
            #         fout0.write(out_str)
            #         fout0.close()
            ...
            #         filename = outdir +'prroc/'+ str(rs)+'all-average-curves.csv'
            #         with open(filename,"w") as f:
            #             f.write("\n".join(",".join(map(str, x)) for x in (fpr,tpr,precision, recall)))
...
# prroc_conds= ['sages-tpotsk', 'fp-tpotsk', 'drug_features-tpotsk', 'targetsall-tpotdefault']

# get_prroc(prroc_conds, 1, 0, 0.8,classifiers, cl, outdir, write=True)
outdir = 'ensemble_train_801010/'
prroc_conds = ['all-tpotdefault']
get_prroc_averages(prroc_conds,classifiers, cl, outdir, 0, write=True)
